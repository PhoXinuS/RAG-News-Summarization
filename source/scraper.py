...
class Scraper:
    ...
    def scrape(self, start_date: date, end_date: Optional[date] = None) -> Optional[str]:
        ...
        print_colored("🔍 Fetching articles...", "title")
        articles = self._get_article_links(sitemap_url, start_date, end_date)

        if not articles:
            print_colored("❌ No articles found in specified date range.", "error")
            return None

        print_colored(f"✅ Found {len(articles)} articles. Fetching content...", "response")
        results = []
        for i, article in enumerate(articles, 1):
            ...
            print_colored(f"📄 Fetched {i}/{len(articles)} articles.", "response")

        print_colored("\n✨ All articles saved successfully ✨", "title")
    ...
    def _get_article_content(self, article_url: str) -> Dict:
        ...
        try:
            ...
            print_colored(f"Error fetching content from {article_url}: {str(e)}", "error")
            return {
                "title": "Error",
...
if __name__ == "__main__":
    scraper = Scraper()
    result_file = scraper.scrape(date(2025, 2, 15))
    if result_file:
        print_colored(f"Results saved to: {result_file}", "response")